...
import { profile, summary, aboutFacts } from '../content';
        ...
        <div className="grid grid-cols-1 lg:grid-cols-12 gap-12 lg:gap-16 items-center">
          ...
          >
            ...
            <motion.div
              ...
              className="text-xs sm:text-sm md:text-[14.5px] font-light text-[#B3A497] leading-[1.85] tracking-wide mb-10 max-w-2xl space-y-4"
              style={{ fontFamily: "'Montserrat', sans-serif" }}
            >
              <p>
                I'm <span className="text-[#F3DBB3] font-medium">{profile.name}</span>. {summary[0]}
              </p>
              <p>{summary[1]}</p>
            </motion.div>
            ...
            <motion.div
              variants={fadeUpVariants}
              className="flex flex-wrap gap-2.5 pt-6 border-t border-[#8C6D4F]/25"
            >
              {aboutFacts.map((fact) => (
                <span
                  key={fact}
            ...
            >
              ...
              <div className="relative overflow-hidden w-full max-w-[390px] aspect-[4/5] bg-black rounded-sm">
                ...
                <img
                  src={aboutImg}
                  alt={profile.name}
                  className="w-full h-full object-cover object-[70%_40%] filter brightness-[0.94] contrast-[1.06] saturate-[1.02] group-hover:brightness-105 group-hover:contrast-[1.12] transition-all duration-700 ease-out"
                />
