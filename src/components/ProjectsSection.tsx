...
import { programmes, profile } from '../content';
...
export const ProjectsSection: React.FC = () => {
  ...
  const renderCard = (project: Programme) => (
              <div className="relative w-full rounded-2xl border border-[#8C6D4F]/50 bg-[#0E0C0A] p-6 sm:p-12 shadow-[0_25px_70px_rgba(0,0,0,0.98)] group overflow-hidden transition-colors duration-500 hover:border-[#D4AF37]">
                ...
                <div className="grid grid-cols-1 lg:grid-cols-12 gap-8 items-start relative z-10">
                  ...
                  <div className="lg:col-span-7 flex flex-col justify-between">
                    <div>
                      <div className="flex items-center space-x-3 mb-4">
                        ...
                        <span className="text-[11.5px] font-mono tracking-[0.25em] uppercase text-[#A8988B]">
                          {project.category}
                        </span>
                      </div>
                      ...
                      <span
                        className="block text-[11.5px] font-medium tracking-[0.22em] uppercase text-[#8C6D4F] mb-4"
                    ...
                    <div className="flex flex-wrap gap-2 pt-6 border-t border-[#8C6D4F]/25">
                      {project.tags.map((t) => (
                        <span
                          key={t}
                          className="px-3 py-1 text-[11px] font-medium tracking-[0.16em] uppercase rounded-sm border border-[#8C6D4F]/40 bg-[#16120E] text-[#E8D7C5] group-hover:border-[#D4AF37]/50 transition-all duration-300"
                          style={{ fontFamily: "'Montserrat', sans-serif" }}
                        >
                          {t}
                        </span>
                  ...
                  <div className="lg:col-span-5 flex flex-col justify-between h-full space-y-6 lg:pl-6 lg:border-l lg:border-[#8C6D4F]/25">
                    <div className="space-y-3">
                      <span className="text-[11px] font-mono tracking-[0.25em] uppercase text-[#8C6D4F] block mb-2">
                        // PROGRAMME METRICS
                      </span>
                      {project.metrics.map((m) => (
                        ...
                        >
                          <span className="text-[11px] font-mono text-[#A8988B]">
                            {m.label}
                          </span>
  ...
  return (
    <section
      id="work"
      className="relative w-full overflow-x-clip bg-black text-[#E8DFD8] font-sans selection:bg-[#cbb59d] selection:text-black pt-20 pb-32 px-6 sm:px-12 lg:px-20"
    >
      ...
      <div className="max-w-7xl mx-auto w-full relative z-10">
        ...
        {/* Stacking deck on desktop; a plain list on phones and tablets, where
            pinned cards would cover each other and leave empty screens. */}
        {stackCards ? (
          <ScrollStack
            itemDistance={20}
            itemScale={0.035}
          ...
          >
            {programmes.map((project) => (
              <ScrollStackItem key={project.title}>{renderCard(project)}</ScrollStackItem>
            ))}
          </ScrollStack>
        ) : (
          <div className="space-y-6">
            {programmes.map((project) => (
              <div key={project.title}>{renderCard(project)}</div>
            ))}
          </div>
        )}
